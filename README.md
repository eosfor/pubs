...
Установка prerelease из PowerShell Gallery:
```pwsh
Install-Module pubs -Scope CurrentUser -AllowPrerelease -RequiredVersion 0.1.3-beta1

# Import-Module -RequiredVersion не понимает prerelease (только System.Version),
# поэтому импортируйте по пути установленного модуля:
...
- Что делают pipeline:
  - выполняют полный gate: build + xUnit + Pester + генерация help;
  - упаковывают модуль в zip (`out/pubs.<version>.zip`);
  - публикуют zip в GitHub Releases;
  - публикуют модуль в PowerShell Gallery (опционально через `publish_to_gallery`).
- Триггеры:
  - `publish-beta.yml`: push beta-тега `vX.Y.Z-beta.N` или ручной запуск (`workflow_dispatch`);
  - `publish-release.yml`: push стабильного тега `vX.Y.Z` или ручной запуск с тегом.
- Версионирование (без ручного редактирования манифеста в git):
  - в pipeline меняется только staging-манифест в `out/pubs/<version>/pubs.psd1`;
  - stable: `X.Y.Z`;
  - beta-tag: `X.Y.Z-beta.N`, где `X.Y.Z` должно быть больше последней stable-версии (обычно следующий patch);
  - beta в PSGallery: `X.Y.Z-betaN` (ограничение `Prerelease` в `Update-ModuleManifest`: только буквы/цифры);
  - это исключает конфликт папок модуля между stable и beta на одной базе `X.Y.Z`.
- Нужные secrets:
...
## Упаковка модуля
Скрипт складывает модуль в `out/pubs/<version>` вместе с зависимостями:
```pwsh
