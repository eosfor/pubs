...
- Триггеры:
  - `publish-beta.yml`: только ручной запуск (`workflow_dispatch`) с `ref` и автоматическим вычислением prerelease версии (`X.Y.Z-beta.N`);
...
- Версионирование (без ручного редактирования манифеста в git):
  ...
  - stable: `X.Y.Z`;
  - beta: `X.Y.Z-beta.N` (для установки из PSGallery нужен `-AllowPrerelease`).
- Нужные secrets:
