...
```
New-SBSubscription -ServiceBusConnectionString <String> -Topic <String> [-Subscription] <String>
 [-RequiresSession <Boolean>] [-EnableBatchedOperations <Boolean>]
 ...
 [-UserMetadata <String>] [-SqlFilter <String>] [-ProgressAction <ActionPreference>] [-WhatIf] [-Confirm]
 [<CommonParameters>]
```
...
```powershell
PS C:\\> New-SBSubscription -ServiceBusConnectionString '<connection-string>' -Topic '<topic-name>' -Subscription '<subscription-name>'
```

Runs New-SBSubscription using the '__AllParameterSets' parameter set.
